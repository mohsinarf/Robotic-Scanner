...
char* szNetworkErrorMessage (int32 lErrorCode)
    ...
    FormatMessage (FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ALLOCATE_BUFFER, NULL, lErrorCode, MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), (LPTSTR)&szErrorText, 0, NULL);
...
char* szIPFromName (const char* szName)
    ...
    strncpy (szIPBuffer, inet_ntoa (ipv->sin_addr), 16);
...
int lInet_pton (const char* szHostname, struct sockaddr_in* pstSockAddr)
    {
#ifdef WIN32
    int lAddrLen = sizeof (*pstSockAddr);
    if (WSAStringToAddressA (szIPFromName (szHostname), AF_INET, NULL, (struct sockaddr*)pstSockAddr, &lAddrLen) == SOCKET_ERROR)
        {
        return SOCKET_ERROR;
        }
#else
